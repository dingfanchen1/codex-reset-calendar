...
S1 本地验证、S2 发布准备、S3 公共服务验证和 S4 iPhone 实机验收均已有执行记录。按 S1 → S2 → S3 → S4 依次记录，不能以计划步骤当作证据。
...
- 固定事件样例已自动核对 UTC、UID、版本、中文、来源与两个 VALARM；两个提醒也已在 S4 由 iPhone 实机观察通过。
...
- 设备：iPhone 13，iOS 26.6.1，时区为中国北京，使用 Wi-Fi；生产和测试订阅的“日程提醒”已开启。
- 首次订阅：用户确认生产日历名称为“Codex 重置日历”，当前为空；测试事件显示在北京时间 2026-09-11 11:00，首次实际看到时刻为 2026-09-10 19:20。
- 同 UID 改期：提交 `42afc26` 把 `s4-visibility-1` 从北京时间 11:00 改为 12:00，UID 不变且 `SEQUENCE` 从 0 递增为 1；运行 [34471295564](https://github.com/dingfanchen1/codex-reset-calendar/actions/runs/34471295564) 成功。用户在 2026-09-10 19:35 刷新日历后确认 11:00 已空、只有 12:00，且未重新订阅、没有重复事件。
- 新 UID 发布：提交 `3c27a77` 把测试源替换为 `s4-replacement-2`，北京时间 13:00，`SEQUENCE=0`；运行 [34473394417](https://github.com/dingfanchen1/codex-reset-calendar/actions/runs/34473394417) 成功。线上只有新 UID，测试 ICS 与本地 SHA-256 均为 `d1d2ec3f7f7ae039df9644b46336d9e88ed92d70aeaae7998cefe88ccd9ee616`，生产 ICS 字节不变。
- 新 UID 实机结果：用户在 2026-09-10 20:09 确认原订阅无需重新添加，12:00 已空、只有 13:00 的新事件。
- 清空发布：提交 `0640d4f` 发布零事件测试日历；运行 [34513193853](https://github.com/dingfanchen1/codex-reset-calendar/actions/runs/34513193853) 成功。线上测试日历保留名称且 `VEVENT=0`，测试 ICS 与本地 SHA-256 均为 `67873ac6a63c21f868703e23b298c4abbf4f8cb0964281308d69f74d261aa889`，生产 ICS 字节不变。
- 清空实机结果：用户在 2026-09-11 02:24 确认 13:00 事件已消失，测试订阅仍在日历列表，且没有重新订阅。
- 双提醒事件发布：用户选择北京时间 2026-09-11 16:58；在 02:33 生成 `sourceId=s4-notification-3`，对应 UTC `2026-09-11T08:58:00Z`，距事件约 14 小时 25 分钟。提交 `09200a5` 已发布，运行 [34515437197](https://github.com/dingfanchen1/codex-reset-calendar/actions/runs/34515437197) 成功；线上只有一个新 UID、`SEQUENCE=0`，并含 `TRIGGER:-PT15M` 与 `TRIGGER:PT0S`。线上测试 ICS 与本地 SHA-256 均为 `5177c11f82f3ef7c51e5f355427573d26d7ab16771a821da412e564ba3a440f5`，生产 ICS 仍为空且字节不变。
- 双提醒事件实机同步：用户在 2026-09-11 02:50 确认 16:58 事件已经显示，且没有重新订阅；事件在 16:43 提前提醒前约 13 小时 53 分钟已同步。
- 双提醒实机结果：用户确认 2026-09-11 16:43 实际收到提前 15 分钟通知，16:58 实际收到到点通知，两次观察时间均与预期一致。
- 收尾清空发布：2026-09-11 17:57 生成零事件测试日历，提交 `d943a39` 已发布；运行 [34586886511](https://github.com/dingfanchen1/codex-reset-calendar/actions/runs/34586886511) 的测试、部署和两个部署后字节核对均成功。线上测试日历恢复为零事件，SHA-256 为 `67873ac6a63c21f868703e23b298c4abbf4f8cb0964281308d69f74d261aa889`；生产日历仍为零事件，SHA-256 保持 `a7fbb8794f69de93e0d00105a83afc6def846df682f5610ed258a4a8602b1925`。
- S4 结论：生产订阅、时区、同 UID 改期、新 UID 替换、清空、提前同步、提前 15 分钟通知和到点通知均有 iPhone 实机证据，测试过程中未重新订阅，生产源未写入测试数据。iPhone 实机验收通过。
...
| 项目 | 期望 | 实际结果 | 时间/证据 | 状态 |
|---|---|---|---|---|
| 生产订阅 | 名称、事件、时区正确 | 名称正确；当前空日历；测试事件正确显示为北京时间 11:00 | iPhone 13 / iOS 26.6.1；2026-09-10 19:20 首次看到 | 通过（当前上游无事件） |
| 同 UID 改期 | 无重复，时间更新 | 11:00 已空，只有 12:00；没有重复 | 运行 34471295564；2026-09-10 19:35 iPhone 刷新后确认 | 通过 |
| 新 UID 替换 | 只有新事件 | 12:00 已空，只有 13:00 新事件；未重新订阅 | 运行 34473394417；2026-09-10 20:09 iPhone 确认 | 通过 |
| 清空 | 无事件，订阅仍在 | 13:00 事件已消失，测试订阅仍在；未重新订阅 | 运行 34513193853；2026-09-11 02:24 iPhone 确认 | 通过 |
| 双提醒事件同步 | 提前 15 分钟之前可见 | 16:58 事件已显示；未重新订阅 | 2026-09-11 02:50 iPhone 确认 | 通过 |
| 提前 15 分钟 | 实际通知一次 | 16:43 实际收到通知 | 2026-09-11 16:43 iPhone 确认 | 通过 |
| 到点 | 实际通知一次 | 16:58 实际收到通知 | 2026-09-11 16:58 iPhone 确认 | 通过 |
