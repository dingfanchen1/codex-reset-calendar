...
- S4：iPhone 实机验收通过；生产订阅、改期、替换、清空、提前同步及两次实际通知均有证据，测试日历已恢复为空。
...
2026-09-11 S4 iPhone 实机核验：iPhone 13 / iOS 26.6.1 / 北京时区 / Wi-Fi 下，生产订阅和独立测试订阅名称、时区与可见性正确；同 UID 改期无重复，新 UID 替换和清空无需重新订阅；16:43 提前 15 分钟通知与 16:58 到点通知均实际收到。收尾运行 `34586886511` 成功，线上生产与测试日历均为零事件且与本地字节一致。详细记录见 Manual_Verification_Guide.md。

## 下一阶段

四个阶段均已完成。后续按运维记录观察定时同步；真实上游出现新事件时，再核对生产订阅的实际到达延迟。
...
### S4 iPhone 订阅与双提醒实机验收（已完成）

- 完成内容：在 iPhone 13 上核对生产订阅；使用独立测试源完成首次可见性、同 UID 改期、新 UID 替换、清空、未来事件提前同步和两次通知；验收结束后把测试日历恢复为空。
- 验证：改期后旧时间消失且无重复，替换和清空无需重新订阅；北京时间 2026-09-11 16:43 与 16:58 分别实际收到提前 15 分钟和到点通知；收尾运行 `34586886511` 成功，线上两个 ICS 均为 HTTP 200、`text/calendar`、零事件且与本地字节一致；41 项 unittest 通过。
- 证据：`Manual_Verification_Guide.md` 的 S4 启动记录与证据表；生产地址和独立测试地址；运行 `34468692967`、`34471295564`、`34473394417`、`34513193853`、`34515437197`、`34586886511`。
- 限制：本次通知证据来自独立测试源；真实上游事件的实际到达时间仍受上游发现延迟和 Apple 订阅刷新延迟影响。
- 下一阶段输入：无开发阶段；按运维记录观察定时同步，真实上游出现事件时记录实际到达延迟。
